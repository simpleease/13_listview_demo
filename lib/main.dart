        ...
        home: Scaffold(
            ...
            body: NotificationListener<ScrollNotification>(// 添加 NotificationListener 作为父容器
              ...
              child: ListView.separated(
                  itemBuilder: (context, index) => ListTile(title: Text("Index : $index"), subtitle: Text("Body $index")),
                  separatorBuilder: (context, index) => (index % 2 == 0) ? Divider(color: Colors.green) : Divider(color: Colors.red),
                  itemCount: 30),
            )
        )
    );
...
class _ScrollControllerState extends State<ScrollControllerWidget> {
  ...
  void initState() {
    ...
    _controller.addListener(() {// 为控制器注册滚动监听方法
      if(_controller.offset > 800) {// 如果 ListView 已经向下滚动了 1000，则启用 Top 按钮
          ...
          Container(
            ...
            child: ElevatedButton(
              onPressed: (isToTop
                  ...
                  : () => print("Top按钮点击忽略")),
              child: Text("Top"),
              style: ButtonStyle(backgroundColor: MaterialStateProperty.all(isToTop ? Colors.blue : Colors.grey)),
            ),
          ),
          Expanded(
            child: ListView.builder(
